...
export default function PeopleSearchPage() {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      setLoading(true)
      userService.searchUsers({ q: search || undefined, role: roleTab !== 'All' ? roleTab : undefined })
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 24, alignItems: 'start' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16, position: 'sticky', top: 100 }}>
              ...
              <GlassCard hover={false} className="p-5">
                ...
                {[['Total users', users.length], ['Frontend', users.filter(u => u.role === 'Frontend').length], ['Backend', users.filter(u => u.role === 'Backend').length], ['ML / AI', users.filter(u => u.role === 'ML / AI').length]].map(([k, v]) => (
