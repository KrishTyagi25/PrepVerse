...
export
  ...
  useEffect(() => {
    setLoading(true)
    const req = tab === 'friends' ? leaderboardService.getFriends() : leaderboardService.getGlobal()
    req.then(({ data }) => setUsers(data.data.users)).catch(() => { }).finally(() => setLoading(false))
  }, [tab])
    ...
    <div style={{ minHeight: '100vh', background: '#080909', color: '#f8fafc', fontFamily: 'Geist,sans-serif' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        ...
        <main style={{ maxWidth: 860, margin: '0 auto', padding: '100px 24px 60px' }}>
          ...
          <TopThreePodium users={loading ? [] : users.slice(0, 3)} />
          ...
          <div style={{ border: '1px solid rgba(255,255,255,.07)', borderRadius: 14, overflow: 'hidden' }}>
            ...
            {loading ? Array.from({ length: 8 }).map((_, i) => <SkeletonRow key={i} />) : users.map((u, i) => <RankRow key={u._id || i} user={{ ...u, rank: i + 1 }} />)}
          </div>
