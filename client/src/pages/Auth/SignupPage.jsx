...
const GOALS = [
  { id: 'campus', label: 'Placement season prep (Campus)' },
  { id: 'offcampus', label: 'Off-campus job search' },
  { id: 'switch', label: 'Career switch / upskill' },
                ...
                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                      {GOALS.map(g => (
                        <label key={g.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 13px', borderRadius: 9, background: 'rgba(255,255,255,.02)', border: '1px solid rgba(255,255,255,.06)', cursor: 'pointer' }}>
                          <input type="radio" name="goal" value={g.id} checked={goal === g.id} onChange={e => setGoal(e.target.value)} style={{ accentColor: '#00d2ff', width: 14, height: 14, flexShrink: 0 }} />
                          <span style={{ fontSize: 13, color: '#94a3b8' }}>{g.label}</span>
                        </label>
                      ))}
                    </div>
                  </div>
